...
function ensure_product_order_schema(mysqli $conn): void
{
    ...
    if ((int) $existing['c'] === 6) { $ready = true; return; }
...
function order_payment_options(mysqli $conn): array
{
    ...
    return [
        'clinic' => true,
        'gcash' => ['enabled' => $name !== '' && preg_match('/^(09\d{9}|\+639\d{9})$/', $number) === 1,
...
function order_create(mysqli $conn, int $client, array $input): array
{
    ...
    $notes = $input['notes'] ?? '';
    if (!in_array($method, ['clinic', 'gcash'], true)) throw new ProductOrderError('Choose GCash or pay at the clinic.', 422);
    ...
    try {
        ...
        $message = $user['full_name'] . ' placed order #' . $id . ' for PHP ' . number_format($total / 100, 2) . '. ' . ($method === 'clinic' ? 'Pay at clinic on pickup.' : 'GCash payment proof is pending.');
        order_notify($conn, $id, $client, 'New product order #' . $id, $message, true);
        order_notify($conn, $id, $client, 'Order #' . $id . ' placed', 'Your products are reserved for clinic pickup. ' . ($method === 'clinic' ? 'Pay at the clinic when collecting your order.' : 'Send the exact total to the GCash recipient shown in your order, then upload payment proof.'));
...
function order_upload_proof(mysqli $conn, int $client, int $id, string $reference, array $file): array
{
    ...
    if (!preg_match('/^\d{10,20}$/', $reference)) throw new ProductOrderError('Enter the 10–20 digit GCash reference number.', 422);
...
function order_save_proof(mysqli $conn, int $client, int $id, string $reference, string $image, string $mime): array
{
    ...
    try {
        ...
        $previous = order_db($conn, 'SELECT order_id,image_hash FROM product_order_proofs WHERE reference=? AND image_hash=?', 'ss', [$reference, $hash])->get_result()->fetch_assoc();
        if ($previous) {
        ...
        if ($order['payment_method'] !== 'gcash' || !in_array($order['status'], ['placed','ready'], true) || !in_array($order['payment_status'], ['pending','rejected'], true)) throw new ProductOrderError('This order is not accepting payment proof.', 409);
        ...
        order_notify($conn, $id, $client, 'Payment proof for order #' . $id, 'A GCash payment proof is awaiting verification. Check the transfer in the clinic GCash account before accepting.', true);
        order_audit($conn, $client, $id, 'Submitted GCash payment proof');
...
function order_record_sale(mysqli $conn, array $order, int $staff, ?float $cashReceived = null): void
{
    ...
    $id = $order['id'];
    $notes = 'Product order #' . $id . ' / ' . ($order['payment_method'] === 'gcash' ? 'GCash (verified by staff)' : 'Pay at clinic');
    ...
    if (order_has_column($conn, 'pos_transactions', 'payment_method')) {
        order_db($conn, "INSERT INTO pos_transactions(client_id,handled_by,total_amount,payment_status,payment_method,notes) VALUES(?,?,?,'paid',?,?)", 'iidss', [(int) $order['client_id'], $staff, $order['total_amount'], $order['payment_method'] === 'gcash' ? 'qr' : 'cash', $notes]);
    } else {
...
function order_update(mysqli $conn, int $actor, int $id, string $action, array $input = [], bool $staff = false): array
{
    ...
    try {
        ...
        if ($action === 'cancel') {
            ...
            if ($order['payment_method'] === 'gcash' && $order['payment_status'] !== 'paid') throw new ProductOrderError('Verify the GCash payment before marking the order ready.', 409);
            ...
            if ($action === 'verify' && ($input['confirmed'] ?? '') !== 'yes') throw new ProductOrderError('Confirm that you checked the reference and exact amount in the clinic GCash account.', 422);
            ...
            $message = $action === 'verify' ? 'Your GCash payment has been verified.' : 'Payment proof was rejected: ' . $note . ' Contact the clinic if money was deducted; do not pay twice.';
