...
<section class="page-heading product-orders-heading"><h1>Product Orders</h1><p>Manage mobile purchases, verify transfer payments, and prepare clinic pickups.</p><a class="btn btn-outline-primary" href="payment_settings.php">Payment settings</a></section>
...
<p><?= badge($detail['status']) ?> <?= badge(str_replace('_',' ', $detail['payment_status'])) ?> · <?= $detail['payment_method'] === 'clinic' ? 'Pay at clinic' : ($detail['payment_method'] === 'bank' ? 'Bank transfer' : 'GCash') ?> · <?= e($detail['created_at']) ?></p>
...
<?php if ($detail['payment_method'] !== 'clinic'): $recipient = $detail['payment_recipient']; ?><p>Recipient for this order: <?= e($recipient['provider'] ?? 'GCash') ?> · <?= e($recipient['account_name'] ?? $detail['gcash_name']) ?> · <?= e($recipient['account_number'] ?? $detail['gcash_number']) ?></p><?php if (!empty($recipient['qr_url'])): ?><img src="../<?= e($recipient['qr_url']) ?>" alt="Receiving QR for this order" style="width:240px;max-width:100%;height:240px;object-fit:contain;background:white"><?php endif; endif; ?>
<?php if ($proof): ?><div class="mb-3"><strong>Transfer reference: <?= e($proof['reference']) ?></strong><br><a href="order_proof.php?id=<?= (int) $proof['id'] ?>" target="_blank" rel="noopener">View payment proof</a><br><img src="order_proof.php?id=<?= (int) $proof['id'] ?>" alt="Transfer payment proof for order <?= $detail['id'] ?>" style="max-width:100%;max-height:360px;object-fit:contain;margin-top:12px"></div><?php endif; ?>
<?php if ($open): ?>
...
<label class="d-block mb-3"><input type="checkbox" name="confirmed" value="yes"> I checked the reference, recipient, and exact amount received in the clinic receiving account.</label>
<button class="btn btn-success me-2 mb-2" name="action" value="verify">Verify transfer payment</button><button class="btn btn-outline-danger mb-2" name="action" value="reject">Reject payment proof</button>
<?php endif; ?>
...
<?php while ($row = $rows->fetch_assoc()): ?><tr><td><a href="product_orders.php?id=<?= (int) $row['id'] ?>">#<?= (int) $row['id'] ?></a></td><td><?= e($row['full_name']) ?></td><td>PHP <?= number_format((float) $row['total_amount'],2) ?></td><td><?= $row['payment_method'] === 'clinic' ? 'At clinic' : ($row['payment_method'] === 'bank' ? 'Bank transfer' : 'GCash') ?><br><?= badge(str_replace('_',' ', $row['payment_status'])) ?></td><td><?= badge($row['status']) ?></td><td><?= e($row['created_at']) ?></td></tr><?php endwhile; ?>
